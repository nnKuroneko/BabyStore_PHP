...
$(function () {
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'รายการสินค้าและแบรนด์ภายในปี <?php echo $date = date("Y"); ?>'
        },
        tooltip: {
        ...
        series: [{
            ...
            data: [
				<?php 
				$result = mysqli_query($mysqil, "SELECT brand FROM product Group by brand");
				while($row = mysqli_fetch_array($result)){
				
				$brnd = $row['brand'];
				
				$result1 = mysqli_query($mysqil, "SELECT * FROM product WHERE brand = '$brnd'");
